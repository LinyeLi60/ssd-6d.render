...
def decode_boxes(locations, net_priors):
    ...
    # decode_boxes(encoded_boxes[i], priors)
    # Arguments
        locations: Numpy array of predicted locations. (num_priors, 4)
        net_priors: Numpy array of prior boxes from network.    (num_priors, 8)
...
def process_detection_output(session, priors, nms, models,
    ...
    # Arguments
        ...
        encoded_boxes: Encoded location regression    (1, num_priors, 4)
        cla_probs: Class probabilities of each prior    # (1, num_priors, num_classes)
        view_probs: View ID probabilities of each prior    # (1, num_priors,num_views)
        inpl_probs: Inplane ID probabilities of each prior    # (1, num_priors, num_inpls)
        ...
        所以views_to_parse应该和inplanes_to_parse大小相等
    ...
    for i in range(encoded_boxes.shape[0]):
        ...
        for c in range(1, cla_probs.shape[2]):  # skip background (class 0)
            confs = cla_probs[i, :, c]
            mask = confs > confidence_threshold    # 是否在于confidence阈值
